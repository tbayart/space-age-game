...
namespace SpaceShooter
{
    public class MenuEntry
    {
        ...
        /// <summary>
        /// Does This Menu Entry Have Wobling Effect ?
        /// </summary>
        bool woblingEffect;
        public bool WoblingEffect
        {
            get { return woblingEffect; }
            set { woblingEffect = value; }
        }
        ...
        public virtual void Draw(GameTime gameTime, bool isSelected)
        {
            ...
            // if the wobling effect is on
            if (isSelected && woblingEffect)
            {
                
                entryPosition = new Vector2((float)r.Next((int)position.X - 4, (int)position.X + 4), (float)r.Next((int)position.Y - 4, (int)position.Y + 4));
            }
            else
            {
                 entryPosition = new Vector2(position.X, position.Y);
            }
            ...
            {
                spriteBatch.Draw(entryTexture, position, new Color(Color.White.R, Color.White.G, Color.White.B, menuScreen.ScreenAlpha));
